...
def img2label_paths(img_paths,img_size=(640,512)):
    #rewrite the labels generation
    ...
    for img in img_paths:
        ...
        video=img_tmp[-2]
        file=img_tmp[-1]
        file_idx=int(file.replace(".jpg",""))-1
        with open(os.path.join("dataset/Drone-Detection&Tracking",flag,video,"IR_label.json")) as f:
            label_dict=json.load(f)
        label_file=file.replace(".jpg",".txt")
        os.makedirs(os.path.join("dataset/labels",flag,video),exist_ok=True)
        with open(os.path.join("dataset/labels",flag,video,label_file),"w+",encoding='utf-8') as f:
            label_list.append(os.path.join("dataset/labels",flag,video,label_file))
            if label_dict["exist"][file_idx]==1:
                lx,ly,origin_w,origin_h=label_dict["gt_rect"][file_idx]
                cx=(lx+origin_w/2)/w
                cy=(ly+origin_h/2)/h
                rw=origin_w/w
                rh=origin_h/h
                f.write(f"0 {cx} {cy} {rw} {rh}\n")
    return label_list
